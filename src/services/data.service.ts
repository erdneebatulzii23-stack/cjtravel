...
export class DataService {
  ...
  private loadFromStorage() {
      // Server дээр ажиллаж байвал (localStorage undefined) шууд буцна
...
### 2. Main.ts файлыг сайжруулах (Алдаа илрүүлэх)

Мөн `src/main.ts` файлыг доорх кодоор солиорой. Энэ нь хэрэв апп гацвал дэлгэц дээр шууд алдааг нь харуулж өгөх тул оношлоход амар болно.
...
// Алдааг дэлгэц дээр харуулах функц
  ...
  if (root) {
    root.innerHTML = `
      <div style="padding: 20px; font-family: sans-serif; color: #721c24; background-color: #f8d7da; border: 1px solid #f5c6cb; border-radius: 5px;">
        ...
        <p>Алдаа гарлаа. Доорх мэдээллийг хөгжүүлэгчид илгээнэ үү:</p>
        <pre style="background: #fff; padding: 10px; border: 1px solid #ddd; overflow: auto;">${error.message || error}</pre>
        <pre style="font-size: 10px; color: #555;">${error.stack || ''}</pre>
      </div>
    `;
  }
}
